...
public class ExternalContactRepository @Inject constructor(
    ...
    public suspend fun discover(
        ...
        val body = if (value.contains("@")) {
            DiscoverExternalContactBody(organizationId = organizationId, email = value)
        } else {
            DiscoverExternalContactBody(organizationId = organizationId, phone = value)
        }
        return domainApi.discoverExternalContact(body).unwrap()
    }
