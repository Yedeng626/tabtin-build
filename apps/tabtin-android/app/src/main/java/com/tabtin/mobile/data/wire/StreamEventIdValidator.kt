// ════════════════════════════════════════════════════════════════════
// W4.5 第二波 B3 · `isStreamEventId` 跨语言契约 SSOT（Kotlin 占位实现）
//
...
//
// W6 Android 实施 Agent 启动时：
//   import com.tabtin.mobile.data.wire.StreamEventIdValidator
//
//   **两个使用点都必须经过本 validator**（缺一不可，缺一会产生不同形态的
//   续传沉默失效）：
//
//   ① 冷启动持久化：`LastEventIdPersistence`（待 W6 实现）写
//      SharedPreferences 前调本 validator —— 否则 `evt_<uuid>` 老协议
//      cursor 污染存储，下次冷启动 backend `_handle_resume` 走 replay=0
//      沉默路径，**用户感知不到 cursor 失效**。
//   ② 运行时内存追踪：`WebSocketService.lastEventIdPerTopic`（参见
//      apps/tabtin-android/.../websocket/WebSocketService.kt:149，**当前未做
//      过滤** —— W6 必须补上）写入前调本 validator —— 否则网断 30s 重连
...
//
// 实现要点：
//   - 必须用 `c in '0'..'9'` 显式 ASCII 范围判断，**禁止用 `c.isDigit()`**
//     —— Kotlin 的 `Char.isDigit()` 接受 Unicode 数字（全角 / 阿拉伯-印度），
//     与 fixture 冲突。
//   - 用 `String.split(...).` 默认行为可能保留空段（"-" -> ["",""]），所以
//     强制断言 head/tail 都非空。
//
// 注意：本文件不是 wire-codegen 自动生成的（codegen 基于 stream-content-block.ts
// zod SSOT，本契约不是结构化数据 schema 而是函数行为契约）。属于"手写常驻
// 占位"，放在 generated/kotlin/ 路径以便 W6 Android 直接 vendor in。
// ════════════════════════════════════════════════════════════════════

package com.tabtin.mobile.data.wire

public object StreamEventIdValidator {
     ...
     */
    @JvmStatic
    public fun isStreamEventId(id: String?): Boolean {
        ...
        return head.all { it in '0'..'9' } && tail.all { it in '0'..'9' }
    }
}
