...
static jbyteArray PairingContext_Encrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    ...
    size_t written_sz;
    int status = EVP_AEAD_CTX_seal(aes_ctx, out, &written_sz, out_size, nonce, nonce_size, (uint8_t *) in_buf, in_size, nullptr, 0);
...
static jbyteArray PairingContext_Decrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    ...
    size_t written_sz;
    int status = EVP_AEAD_CTX_open(aes_ctx, out, &written_sz, out_size, nonce, nonce_size, (uint8_t *) in_buf, in_size, nullptr, 0);
    env->ReleaseByteArrayElements(jIn, in_buf, 0);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
    ...
    env->RegisterNatives(
            env->FindClass("com/tabtin/mobile/data/adb/PairingContext"),
            methods,
