...
if [ "$PROFILE" = "community" ]; then
  ...
  # 自托管/社区档：关闭手机号「新注册」（既有手机号账号仍可登录/找回密码）。
  # 需要放开时把 TABTIN_COMMUNITY_PHONE_REGISTER_ENABLED 设成 true 再构建。
  export VITE_AUTH_PHONE_REGISTER_ENABLED="${TABTIN_COMMUNITY_PHONE_REGISTER_ENABLED:-false}"
...
case "$PROFILE" in
  local)
    PROFILE_PRODUCT_NAME="QiZhiKitty Local"
    PROFILE_APP_ID="com.qizhikitty.app.local"
    PROFILE_EXECUTABLE_NAME="qizhikitty-local"
    PROFILE_SHORTCUT_NAME="QiZhiKitty Local"
    ;;
  community)
    PROFILE_PRODUCT_NAME="QiZhiKitty"
    PROFILE_APP_ID="com.qizhikitty.community"
    PROFILE_EXECUTABLE_NAME="qizhikitty"
    PROFILE_SHORTCUT_NAME="QiZhiKitty"
    ;;
esac
...
ELECTRON_BUILDER_PACKAGE_JSON="$(node_path "$APP_DIR/node_modules/electron-builder/package.json")"
ELECTRON_BUILDER_CLI="$(node_path "$APP_DIR/node_modules/electron-builder/cli.js")"
...
INSTALLED_ELECTRON_PACKAGE_JSON="$(node_path "$APP_DIR/node_modules/electron/package.json")"
INSTALLED_ELECTRON_DIST="$(node_path "$APP_DIR/node_modules/electron/dist")"
