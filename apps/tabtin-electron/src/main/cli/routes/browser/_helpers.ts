...
export function requireBridgeAndSpace(
  ...
  if (!bridge) {
    sendJSON(
      ...
      errorResponse(
        'INTERNAL_ERROR',
        'QiZhiKitty 界面尚未就绪，请确保应用窗口已打开',
        {
          retryable: true,
          suggestions: ['确保 QiZhiKitty 主窗口已显示', '等待几秒后重试'],
        },
      ),
    )
    return null
  ...
  if (!spaceId) {
    sendJSON(
      ...
      errorResponse(
        'VALIDATION_ERROR',
        '未选择组织，请先在 QiZhiKitty 中打开一个 Space',
        {
          suggestions: ['在 QiZhiKitty 中创建或选择一个 Space'],
        },
      ),
    )
    return null
...
export function enhanceErrorResponse(
  ...
  if (msg.includes('未注册的工具') || msg.includes('unregistered tool')) {
    sendJSON(
      ...
      errorResponse(
        'VALIDATION_ERROR',
        '该功能当前不可用，请确保 QiZhiKitty 已更新到最新版本',
        {
          suggestions: [
            '请更新 QiZhiKitty 到最新版本',
  ...
  if (msg.includes('尚未初始化') || msg.includes('not initialized')) {
    sendJSON(
      ...
      errorResponse(
        'INTERNAL_ERROR',
        'QiZhiKitty 正在启动中，请稍后重试（通常需要 5-10 秒）',
        {
          retryable: true,
          suggestions: ['等待几秒后重试', '确保 QiZhiKitty 应用已完全启动'],
          detail: { original: msg },
        },
      ),
    )
