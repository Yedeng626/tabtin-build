...
export async function handleDesktopRoute(
  ...
  if (
    ...
    sendJSON(res, statusFromErrorCode(DesktopErrorCode.UNSUPPORTED_PLATFORM), desktopErrorPayload(
      ...
      `如需桌面操控，请在 macOS 或 Windows 上运行 QiZhiKitty 客户端。`,
    ))
    return
  }
  ...
  if (!executor) {
    sendJSON(res, 503, desktopErrorPayload(
      ...
      `桌面操控暂不可用：执行器尚未初始化。本次请求未执行。请确认 QiZhiKitty 桌面客户端已完全启动后重试。`,
      { suggestions: ['等待 QiZhiKitty 桌面客户端完全启动后再发起请求'] },
    ))
    return
  }
    ...
    switch (route) {
      ...
      case '/open': {
        ...
        if (!allowExternal && isExternalTerminalAppName(appName)) {
          ...
          sendJSON(res, statusFromErrorCode(code), desktopErrorPayload(
            code,
            `「${appName}」是系统终端。用户说「打开终端」应打开 QiZhiKitty 应用内终端，请改用：tabtin terminal open。若用户明确要求外部系统终端，请加 --external：tabtin desktop open "${appName}" --external。`,
            {
              suggestions: [
    ...
    sendJSON(
      ...
      desktopErrorPayload(
        ...
        code === DesktopErrorCode.TCC_DENIED
          ? { suggestions: ['在 macOS 系统设置 → 隐私与安全 → 辅助功能 中启用 QiZhiKitty'] }
          : undefined,
      ),
    )
  }
