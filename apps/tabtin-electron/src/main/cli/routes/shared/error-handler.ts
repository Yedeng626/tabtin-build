...
const ELECTRON_SUGGESTIONS: Partial<Record<string, string[]>> = {
  UNAUTHORIZED: ['请先登录 QiZhiKitty 应用', '确保在 QiZhiKitty 内置终端中运行命令'],
  AUTH_EXPIRED: ['登录已过期，请重新打开 QiZhiKitty 应用', '应用会自动刷新登录状态'],
  PERMISSION_DENIED: ['当前账号没有访问该资源的权限', '请确认你拥有对应组织或 Space 的访问权限'],
  QUOTA_EXCEEDED: ['配额已用尽，请在 QiZhiKitty 设置中查看详情'],
...
export async function djangoRequest(
  ...
  if (!accessToken) {
    return {
      status: 401,
      data: errorResponse('UNAUTHORIZED', '未登录，请先登录 QiZhiKitty'),
    }
  }
