      ...
      if (process.platform === 'darwin' && !session.screenRecordingChecked) {
        ...
        if (!granted) {
          throw new DesktopError(
            ...
            `请前往「系统设置 → 隐私与安全性 → 屏幕录制」允许 QiZhiKitty，` +
            `或运行 tabtin desktop accessibility --prompt 打开系统引导对话框，` +
            `授权后重启 QiZhiKitty 或重新运行 tabtin desktop screenshot。`,
          )
        }
      }
      throw new DesktopError(
        ...
        (process.platform === 'darwin'
          ...
          : `目标显示器可能不可用或已断开。`) +
        `本次截屏未执行。` +
        `请确认显示器在线；macOS 上还需在「系统设置 → 隐私与安全性 → 屏幕录制」中允许 QiZhiKitty。`,
      )
    }
  ...
  private async requireAllowedApp(screenX: number, screenY: number): Promise<void> {
    ...
    if (!app) {
      throw new DesktopError(
        ...
        (process.platform === 'darwin'
          ? `请在「系统设置 → 隐私与安全性 → 辅助功能」中允许 QiZhiKitty；如系统未安装 python3 也会触发此错误，可通过 brew install python 或系统包管理器补装；`
  ...
  private requireAccessibility(): void {
    if (process.platform === 'darwin' && !systemPreferences.isTrustedAccessibilityClient(false)) {
      throw new DesktopError(
        ...
        `请前往「系统设置 → 隐私与安全性 → 辅助功能」允许 QiZhiKitty，` +
