...
export const LoginForm: React.FC<LoginFormProps> = ({
  ...
  const identifierMode = resolveAuthIdentifierMode(AUTH_EMAIL_LOGIN_ENABLED, AUTH_PHONE_LOGIN_ENABLED)
  const identifierCopy = resolveAuthIdentifierCopy(identifierMode)
  const identifierAttrs = resolveAuthIdentifierInputAttrs(identifierMode)
  // 手机号注册关闭后，手机号走验证码登录不再自动建号 → 提示文案随之切换
  const autoRegisterHintKey =
    identifierMode !== 'phone' && !AUTH_PHONE_REGISTER_ENABLED
      ? 'loginForm.hints.autoRegisterOnCodeLoginNoPhoneRegister'
      : identifierCopy.loginAutoRegisterHint
  ...
  return (
    ...
    >
      ...
      <Form onSubmit={form.submit}>
        ...
        {/* 登录标识符输入（密码 / 验证码登录共用；按模式收邮箱或手机号） */}
        <FormField>
          <FormLabel htmlFor="username">
            {t(identifierCopy.loginLabel)}
          </FormLabel>
          <div className="relative">
            <Input
              id="username"
              type={identifierAttrs.type}
              inputMode={identifierAttrs.inputMode}
              autoComplete={identifierAttrs.autoComplete}
              maxLength={identifierAttrs.maxLength}
              value={form.username}
              onChange={(e) => form.setField('username', e.target.value)}
              placeholder={t(identifierCopy.loginPlaceholder)}
