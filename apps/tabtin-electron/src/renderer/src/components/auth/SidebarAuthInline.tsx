...
import {
  ...
  useRegisterForm,
  SMS_CODE_MAX_LENGTH,
  sanitizeAuthIdentifierInputByMode,
  normalizeAuthIdentifier,
  isValidAuthIdentifierByMode,
  resolveAuthIdentifierMode,
  resolveAuthIdentifierCopy,
  resolveAuthIdentifierInputAttrs,
...
/** 标识符模式（模块级：只由构建期 env 决定，运行期不变） */
const IDENTIFIER_MODE = resolveAuthIdentifierMode(AUTH_EMAIL_LOGIN_ENABLED, AUTH_PHONE_LOGIN_ENABLED)
const IDENTIFIER_COPY = resolveAuthIdentifierCopy(IDENTIFIER_MODE)
const IDENTIFIER_ATTRS = resolveAuthIdentifierInputAttrs(IDENTIFIER_MODE)
/** 三种模式下的英文兜底文案（i18n 缺失时用） */
const IDENTIFIER_DEFAULT_LABEL =
  IDENTIFIER_MODE === 'phone' ? 'Phone' : IDENTIFIER_MODE === 'email' ? 'Email' : 'Email or phone'

/** 注册表单的标识符模式：由「手机号注册开关」决定（与登录解耦） */
const REGISTER_IDENTIFIER_MODE = resolveAuthIdentifierMode(AUTH_EMAIL_LOGIN_ENABLED, AUTH_PHONE_REGISTER_ENABLED)
const REGISTER_IDENTIFIER_COPY = resolveAuthIdentifierCopy(REGISTER_IDENTIFIER_MODE)
const REGISTER_IDENTIFIER_ATTRS = resolveAuthIdentifierInputAttrs(REGISTER_IDENTIFIER_MODE)

/**
 * 登录页「验证码登录会自动注册」提示。
 * 手机号注册关闭后，手机号走验证码登录不会再自动建号，换一条更准确的文案。
 */
const LOGIN_AUTO_REGISTER_HINT_KEY =
  IDENTIFIER_MODE !== 'phone' && !AUTH_PHONE_REGISTER_ENABLED
    ? 'loginForm.hints.autoRegisterOnCodeLoginNoPhoneRegister'
    : IDENTIFIER_COPY.loginAutoRegisterHint
...
const LoginPanel: React.FC<{
  ...
  const form = useLoginForm({
    ...
    phoneLoginEnabled: AUTH_PHONE_LOGIN_ENABLED,
  })
  ...
  return (
    ...
    >
      ...
      <FieldWithIcon icon={IDENTIFIER_MODE === 'phone' ? <Smartphone className="h-3.5 w-3.5" /> : <Mail className="h-3.5 w-3.5" />}>
        <Input
          type={IDENTIFIER_ATTRS.type}
          inputMode={IDENTIFIER_ATTRS.inputMode}
          maxLength={IDENTIFIER_ATTRS.maxLength}
          value={form.username}
          onChange={e => form.setField('username', e.target.value)}
          placeholder={t(IDENTIFIER_COPY.loginPlaceholder, { defaultValue: IDENTIFIER_DEFAULT_LABEL })}
          className="pl-9 text-body"
          autoComplete={IDENTIFIER_ATTRS.autoComplete}
        />
      </FieldWithIcon>
      ...
      {form.method === 'password' ? (
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-caption text-muted-foreground/80 leading-relaxed">
            {t(LOGIN_AUTO_REGISTER_HINT_KEY, {
              defaultValue:
                IDENTIFIER_MODE === 'phone'
                  ...
                  : AUTH_PHONE_REGISTER_ENABLED
                    ...
                    : 'Code sign-in creates an account for new email addresses; phone numbers must be registered first.',
            })}
          </p>
        </div>
      )}
...
const RegisterPanel: React.FC<{ onSwitchLogin: () => void }> = ({
  ...
  const form = useRegisterForm({
    ...
    extractError: (err, key) => extractErrorMessage(err, key, undefined, 'auth'),
    emailLoginEnabled: AUTH_EMAIL_LOGIN_ENABLED,
    phoneLoginEnabled: AUTH_PHONE_REGISTER_ENABLED,
  })
  ...
  return (
    <motion.form
      ...
      className="space-y-3"
    >
      <FieldWithIcon icon={REGISTER_IDENTIFIER_MODE === 'phone' ? <Smartphone className="h-3.5 w-3.5" /> : <Mail className="h-3.5 w-3.5" />}>
        <Input
          type={REGISTER_IDENTIFIER_ATTRS.type}
          value={form.phone}
          onChange={e => form.setField('phone', e.target.value)}
          placeholder={t(REGISTER_IDENTIFIER_COPY.registerPlaceholder, {
            defaultValue:
              REGISTER_IDENTIFIER_MODE === 'phone'
                ? 'Phone'
                : REGISTER_IDENTIFIER_MODE === 'email'
                  ? 'Email'
                  : 'Email or phone',
          })}
          maxLength={REGISTER_IDENTIFIER_ATTRS.maxLength}
          className="pl-9 text-body"
          inputMode={REGISTER_IDENTIFIER_ATTRS.inputMode}
          autoComplete={REGISTER_IDENTIFIER_ATTRS.autoComplete}
        />
      </FieldWithIcon>
...
const ForgotPanel: React.FC<{ onBackToLogin: () => void }> = ({
  ...
  const updateUsername = (value: string) => {
    setUsername(sanitizeAuthIdentifierInputByMode(value, IDENTIFIER_MODE))
    if (localError) setLocalError(null)
  }
  ...
  const validateResetIdentifier = (): string | null => {
    ...
    if (!id) {
      setLocalError(
        t(IDENTIFIER_COPY.forgotRequired, {
          defaultValue:
            IDENTIFIER_MODE === 'phone'
    ...
    if (!isValidAuthIdentifierByMode(id, IDENTIFIER_MODE)) {
      setLocalError(
        t(
          IDENTIFIER_MODE === 'emailOrPhone' && !id.includes('@')
            ? 'forgotForm.errors.usernameInvalid'
            : IDENTIFIER_COPY.forgotInvalid,
          {
            defaultValue:
              IDENTIFIER_MODE === 'phone'
  ...
  const sendCode = async (): Promise<boolean> => {
    ...
    try {
      ...
      start()
      setSuccessMsg(
        t(IDENTIFIER_COPY.forgotCodeSent, {
          defaultValue:
            IDENTIFIER_MODE === 'emailOrPhone' ? 'Code sent to your email or phone' : 'Code sent',
        }),
      )
      setLocalError(null)
      return true
    ...
    >
      <FieldWithIcon icon={IDENTIFIER_MODE === 'phone' ? <Smartphone className="h-3.5 w-3.5" /> : <Mail className="h-3.5 w-3.5" />}>
        <Input
          ...
          placeholder={t(IDENTIFIER_COPY.forgotPlaceholder, { defaultValue: IDENTIFIER_DEFAULT_LABEL })}
          maxLength={IDENTIFIER_ATTRS.maxLength}
          inputMode={IDENTIFIER_ATTRS.inputMode}
          className="pl-9 text-body"
          autoComplete={IDENTIFIER_ATTRS.autoComplete}
