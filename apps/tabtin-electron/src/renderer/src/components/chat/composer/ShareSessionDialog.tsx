...
import { resolveOrgInternalShareConversationId, resolveExternalShareConversationId } from './resolveOrgInternalShareConversation'
...
const GROUP_LABEL_CLASS = cn(COMPOSER_TEXT_META, 'px-2.5 pb-1 pt-2 font-medium')
...
/**
 * 接收人 = 本组织成员或组织外好友。externalContact 非空即跨组织共享，
 * 后续投递走外部私聊、且不提供「交给同事继续」（续接要求本组织成员）。
 */
interface ShareRecipient extends SearchMemberResult {
  externalContact?: ExternalContact
}

function externalContactToRecipient(contact: ExternalContact): ShareRecipient {
  return {
...
/** 外部接收人显示所属组织名；本组织成员显示 @username。 */
function recipientSubtitle(recipient: ShareRecipient, externalFallback: string): string {
  if (recipient.externalContact) {
    return recipient.externalContact.peer_organization_name || externalFallback
  }
  return recipient.username ? `@${recipient.username}` : ''
}
...
interface RecipientRowProps {
  ...
  disabled: boolean
  subtitle: string
  onPick: (recipient: ShareRecipient) => void
}
  ...
  return (
    ...
    >
      <ColorAvatar
        name={label}
        seed={recipient.id}
        imageUrl={recipient.avatar || undefined}
        className="h-8 w-8"
...
export const ShareSessionDialog: React.FC<Props> = ({
  ...
  // 组织外好友（外部联系人，仅 friend）：跨组织共享的候选来源。
  useEffect(() => {
    ...
    listExternalContacts(organizationId)
      .then((res) => {
        if (cancelled) return
        setExternalContacts((res?.items ?? []).filter((item) => (
          item.relationship === 'friend' && item.peer_user_id
        )))
      })
      .catch((err) => {
        if (cancelled) return
        log.warn('load external contacts failed', { err })
        setExternalContacts([])
      })
      .finally(() => {
  ...
  const externalRecipients = useMemo(
    () => externalContacts
      .map(externalContactToRecipient)
      .filter((recipient) => {
        const keyword = query.trim().toLowerCase()
        if (!keyword) return true
        return (
          recipient.nickname.toLowerCase().includes(keyword)
          || (recipient.externalContact?.peer_organization_name ?? '')
            .toLowerCase()
            .includes(keyword)
        )
      }),
    [externalContacts, query],
  )

  const handlePickRecipient = useCallback((recipient: ShareRecipient) => {
    setSelected(recipient)
    // 组织外接收人没有「转交」档（续接要求本组织成员），选中时回落到共享。
    if (recipient.externalContact) setIntent('participate')
  }, [])

  const canSubmit = Boolean(selected && !submitting && organizationId)
  const allowContinue = !selected?.externalContact
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const conversations = useIMStore.getState().conversations ?? []
      const externalContact = selected.externalContact
      const conversationId = externalContact
        ? await resolveExternalShareConversationId({
          conversations,
          organizationId,
          externalContactId: externalContact.contact_id,
          peerUserId: selected.id,
          createExternalDirect: createExternalDM,
        })
        : await resolveOrgInternalShareConversationId({
          conversations,
          organizationId,
          peerUserId: selected.id,
            ...
            {selected ? (
              <div className="flex items-center gap-2.5 rounded-interactive bg-foreground/[0.04] px-3 py-2 dark:bg-foreground/[0.06]">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {recipientSubtitle(
                    selected,
                    t('shareSession.externalContactFallback', { defaultValue: '外部联系人' }),
                  ) ? (
                ...
                <div className="max-h-44 overflow-y-auto rounded-interactive bg-muted/15">
                  {members.length === 0 && externalRecipients.length === 0 ? (
                    <MemberPickerEmpty
                      ...
                      loading={membersLoading || externalLoading}
                      query={query}
                    />
                  ) : (
                    <>
                      {members.length > 0 ? (
                        <div className={GROUP_LABEL_CLASS}>
                          {t('shareSession.groupOrgMembers', { defaultValue: '本组织成员' })}
                        </div>
                      ) : null}
                      {members.map((member) => (
                        <RecipientRow
                          ...
                          recipient={member}
                          disabled={submitting}
                          subtitle={recipientSubtitle(member, '')}
                          onPick={handlePickRecipient}
                        />
                      ))}
                      {externalRecipients.length > 0 ? (
                        <div className={GROUP_LABEL_CLASS}>
                          {t('shareSession.groupExternalContacts', {
                            defaultValue: '其他组织联系人',
                          })}
                        </div>
                      ) : null}
                      {externalRecipients.map((recipient) => (
                        <RecipientRow
                          ...
                          subtitle={recipientSubtitle(
                            recipient,
                            t('shareSession.externalContactFallback', {
                              defaultValue: '外部联系人',
                            }),
                          )}
                          onPick={handlePickRecipient}
