...
export function buildCreateOrganizationSettings(
  ...
  // 防御：base 理论上总是对象，但历史上出现过 JSON 字符串传入的调用路径，
  // 服务端 `OrganizationCreate.settings: Dict` 会直接以 400
  // 「Input should be a valid dictionary」拒绝整次创建。
  let normalized: unknown = base
  if (typeof normalized === 'string') {
    try {
      normalized = JSON.parse(normalized)
    } catch {
      console.warn('[Organization] settings 既非对象也非合法 JSON，已丢弃')
