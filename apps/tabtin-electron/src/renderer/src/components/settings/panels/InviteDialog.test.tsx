...
const {
  ...
  createEmailTargetedInvitation: vi.fn(),
...
describe('InviteDialog', () => {
  beforeEach(() => {
    ...
    createEmailTargetedInvitation.mockResolvedValue({ token: 'email-token' })
  })

  it('展示邮件邀请入口，并默认展示手机号邀请', () => {
    render(<InviteDialog organizationId="organization-1" onClose={vi.fn()} />)

    expect(screen.getByText('invite.emailTab')).toBeTruthy()
  ...
  it('保留手机号邀请提交路径，且不调用邮件邀请 API', async () => {
    ...
    expect(createEmailTargetedInvitation).not.toHaveBeenCalled()
  })
  ...
  it('按邮箱邀请已注册用户（走 email-targeted 接口）', async () => {
    render(<InviteDialog organizationId="organization-1" onClose={vi.fn()} />)

    fireEvent.click(screen.getByText('invite.emailTab'))
    fireEvent.change(screen.getByPlaceholderText('invite.emailPlaceholder'), {
      target: { value: 'teammate@tabtin.test' },
    })
    fireEvent.click(screen.getByText('invite.sendInvite'))

    await waitFor(() => {
      expect(createEmailTargetedInvitation).toHaveBeenCalledWith('organization-1', 'teammate@tabtin.test', 'editor')
    })
    expect(createEmailInvitation).not.toHaveBeenCalled()
  })

  it('邮箱为空时给出校验提示，不发起请求', async () => {
    render(<InviteDialog organizationId="organization-1" onClose={vi.fn()} />)
    ...
    await waitFor(() => {
      expect(screen.getByText('invite.emailRequired')).toBeTruthy()
    })
    expect(createEmailTargetedInvitation).not.toHaveBeenCalled()
  })
})
