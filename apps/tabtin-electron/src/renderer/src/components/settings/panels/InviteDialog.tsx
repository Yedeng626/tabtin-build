...
import { Link2, Copy, Check, UserPlus, Smartphone, Mail } from 'lucide-react'
...
import { UI_ASSIGNABLE_ROLES } from '@tabtin/app-shell'
...
type TabType = 'phone' | 'email' | 'link' | 'userId'
...
export const InviteDialog: React.FC<InviteDialogProps> = ({ organizationId, onClose, onInvited }) => {
  ...
  const handleEmailInvite = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      await InvitationApiService.createEmailTargetedInvitation(organizationId, email.trim(), role)
      toast({ title: t('invite.inviteSent') })
      onInvited?.()
      onClose()
    } catch (err) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex gap-1 mb-4 bg-muted rounded-lg p-1">
          ...
          <button
            ...
            className={cn(
              'flex-1 flex items-center justify-center gap-1 px-1.5 py-1.5 rounded-md text-body whitespace-nowrap transition-colors',
              tab === 'email' ? 'bg-background shadow-sm font-medium' : 'text-muted-foreground hover:text-foreground',
            )}
          >
        ...
        {tab === 'email' && (
          <form onSubmit={handleEmailInvite} className="space-y-3">
            <div>
              <label className={cn(SETTINGS_LABEL, 'mb-1 block')}>{t('invite.emailLabel')}</label>
              <Input
                type="email"
                value={email}
                ...
                className={cn('w-full', SETTINGS_CONTROL)}
              />
              <p className={cn(SETTINGS_HINT, 'mt-1')}>{t('invite.emailHint')}</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" type="button" size="form" onClick={onClose} className="flex-1">
                {t('invite.cancel')}
