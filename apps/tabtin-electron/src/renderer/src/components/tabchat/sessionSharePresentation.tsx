...
export function buildSessionShareModeOptions(
  t: TranslateFn,
  options: { allowContinue?: boolean } = {},
): ShareSelectOption<SessionShareMode>[] {
  const shareTierOptions = buildShareTierSelectOptions(t)
  const modes: ShareSelectOption<SessionShareMode>[] = [
    ...shareTierOptions.filter((option) => option.value !== 'fork'),
  ]
  // 组织外接收人不支持「转交」：续接服务要求接收方是本组织成员。
  if (options.allowContinue !== false) {
    modes.push({
      value: 'continue',
...
interface SessionShareModeFieldProps {
  ...
  disabled?: boolean
  /** 组织外接收人只有共享（参与）档；为 false 时隐藏「交给同事继续」。 */
...
export const SessionShareModeField: React.FC<SessionShareModeFieldProps> = ({
  ...
  const options = useMemo(
    () => buildSessionShareModeOptions(t, { allowContinue }),
    [t, allowContinue],
  )
