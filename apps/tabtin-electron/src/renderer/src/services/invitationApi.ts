...
export const resolvePendingInvitationSummary = (
  ...
  if (inv.invite_type === 'email' && (inv.invited_user_id || '').trim()) {
    // 定向邮箱邀请（对方已注册）：昵称 + 邮箱标识，与 phone / direct 同形
    const emailLabel = (inv.email || '').trim()
    if (nickname && emailLabel) return { title: nickname, detail: emailLabel }
    return { title: emailLabel || t('members.directInvitationUnavailable') }
  }
...
export class InvitationApiService {
  ...
  static async createEmailTargetedInvitation(
    organizationId: string,
    email: string,
    role: AssignableRole = 'editor',
    expiresHours: number = 72,
  ): Promise<InvitationInfo> {
    const url = joinApiPath(API_CONFIG.baseURL, `${API_ENDPOINTS.ORGANIZATION_INVITATION.CREATE_EMAIL_TARGETED(organizationId)}`)
    const response = await apiRequest({
      url,
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
