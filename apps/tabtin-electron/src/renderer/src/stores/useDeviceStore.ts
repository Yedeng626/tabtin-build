...
interface DeviceState {
  ...
  registerCurrentDevice: (organizationId: string, options?: { force?: boolean }) => Promise<Device | null>
 ...
 * 判定「该设备在服务端已不存在」。
 *
 * 典型来源：删除组织会级联删除其下设备（服务端 organization_service），
 * 之后心跳固定返回 404「设备不存在或已被删除」，本地却仍认为已注册。
 */
function _isDeviceGoneError(error: unknown): boolean {
  const status = (error as { status?: number } | null)?.status
  if (status === 404) return true
  const msg = error instanceof Error ? error.message : String(error ?? '')
  return /设备不存在|已被删除|device not found|device_not_found/i.test(msg)
}

/** 失效重注册限流：避免心跳周期（30s/60s）反复触发注册风暴。 */
const DEVICE_REREGISTER_MIN_INTERVAL_MS = 60_000
let _lastDeviceReregisterAt = 0

/**
 * 设备被服务端删除后的自愈：清掉本地失效注册态并强制重新注册。
 *
 * 修复场景：删组织级联删设备后，心跳一直 404、本地 registered 恒为 true，
 * 于是「创建组织」持续失败在 DEVICE_REQUIRED（初始化默认 Space 必须绑定执行设备）。
 */
function _recoverFromDeletedDevice(): void {
  const now = Date.now()
  if (now - _lastDeviceReregisterAt < DEVICE_REREGISTER_MIN_INTERVAL_MS) return
  const organizationId =
    useOrganizationStore.getState().selectedOrganization?.id ?? _lastRegisterOrganizationId
  if (!organizationId) return
  _lastDeviceReregisterAt = now
  logger.warn('[Device] 服务端设备已失效，强制重新注册:', organizationId)
  void useDeviceStore.getState().registerCurrentDevice(organizationId, { force: true })
}
...
export function startHeartbeat() {
  ...
  const sendHeartbeat = async () => {
    ...
    DeviceApiService.heartbeat(fingerprint, {
      ...
      // 服务端设备行可能已被删除（删组织会级联删设备），此时心跳会一直 404、
      // 而本地 registered 仍为 true 导致永不重注册 → 创建组织等动作会持续报
      // 「必须绑定执行设备」。这里识别该错误并强制重新注册一次（自己限流）。
      if (_isDeviceGoneError(e)) {
        void _recoverFromDeletedDevice()
        return
      }
...
export const useDeviceStore = create<DeviceState>((set, get) => ({
  ...
  registerCurrentDevice: async (organizationId: string, options?: { force?: boolean }) => {
    ...
    set({
      ...(organizationChanged || options?.force ? { currentDevice: null, registered: false } : {}),
      isLoading: true,
      error: null,
    })
    ...
    try {
      ...
      const device = await register(organizationId, options?.force) as Device
