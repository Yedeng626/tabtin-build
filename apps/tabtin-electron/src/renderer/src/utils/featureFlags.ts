...
import {
  ...
  parsePhoneLoginEnabled,
  parsePhoneRegisterEnabled,
...
/**
 * 登录 / 注册 / 找回密码是否允许手机号作为标识符。
 *
 * 读 VITE_AUTH_PHONE_LOGIN_ENABLED；去掉空白后小写等于 'false' 才关闭。
 * 关闭时只认邮箱（对应 auth-forms 的 email 标识符模式），手机号入口整条下线。
 */
export const AUTH_PHONE_LOGIN_ENABLED: boolean = parsePhoneLoginEnabled(
  import.meta.env.VITE_AUTH_PHONE_LOGIN_ENABLED,
)

/**
 * 是否允许用手机号**新注册**。读 VITE_AUTH_PHONE_REGISTER_ENABLED；等于 'false' 时关闭。
 *
 * 关闭只影响注册与「验证码登录的自动注册」：既有手机号账号仍可正常登录 / 找回密码。
 */
export const AUTH_PHONE_REGISTER_ENABLED: boolean = parsePhoneRegisterEnabled(
  import.meta.env.VITE_AUTH_PHONE_REGISTER_ENABLED,
)
