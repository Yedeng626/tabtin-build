...
actor ExternalContactService {
    ...
    /// 单入参按 "@" 分流：邮箱精确匹配（大小写不敏感）或手机号等价字面量。
    func discover(organizationId: String, identifier: String) async throws -> ExternalContactCandidate {
        let value = identifier.trimmingCharacters(in: .whitespacesAndNewlines)
        var body: [String: Any] = ["organization_id": organizationId]
        body[value.contains("@") ? "email" : "phone"] = value
