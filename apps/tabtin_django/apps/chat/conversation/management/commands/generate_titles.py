...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index, session in enumerate(query.iterator(chunk_size=500), 1):
            try:
                # 显示进度
                ...
                # ：只读真实用户消息；模型由 title_generation scene binding 决定
                result = generate_session_title(session, force=bool(force))
