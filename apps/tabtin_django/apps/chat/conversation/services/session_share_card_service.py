...
def _resolve_cross_org_external_contact(*, owner_user_id: str, peer_user_id: str):
    """owner→peer 的 friend 外部联系人行；没有则返回 None。"""
...
def _resolve_django_direct_conversation(
    *,
    ...
    # 跨组织（跨部门）分享：接收方不是本组织成员时不能走 resolve_or_create_dm
    # （其 create_dm 分支会校验同组织成员），改走外部联系人 DM；
    # create_external_dm 自带"已有跨组织 DM 则复用/激活"。
    is_org_member = OrganizationMember.objects.filter(
        organization_id=organization_id,
        user_id=str(other_user_id),
    ).exists()
    if not is_org_member:
        if not session_share_service.organization_allows_cross_org_share(organization_id):
            raise ValueError(session_share_service.GRANTEE_NOT_MEMBER_MESSAGE)
        contact = _resolve_cross_org_external_contact(
            owner_user_id=str(actor_user_id),
            peer_user_id=str(other_user_id),
        )
        if contact is None:
            raise ValueError(session_share_service.GRANTEE_NOT_MEMBER_MESSAGE)
        conversation = ConversationService.create_external_dm(
            organization_id=str(organization_id),
            creator_id=str(actor_user_id),
            external_contact_id=str(contact.id),
        )
...
def accept_and_refresh_card(*, actor_user, share_id: str) -> dict:
    ...
    if not session_share_service.is_share_grantee_eligible(
        share=share,
...
def _serialize_v2_detail(
    ...
    *,
    ...
    if role == "recipient":
        eligible = eligible and session_share_service.is_share_grantee_eligible(
            share=share,
