...
# ── 跨组织（跨部门）分享开关 ────────────────────────────────────────────────
# 组织 settings 里的布尔键：允许本组织的会话共享给**组织外**成员。
# 默认放开：键缺失 / settings 为空一律按允许处理，只有显式写 false 才关闭。
# 自托管部署里「部门 = 独立组织」时，默认放开即等于允许跨部门分享。
CROSS_ORG_SHARE_SETTING_KEY = "allow_cross_org_share"

GRANTEE_NOT_MEMBER_MESSAGE = "接收人不是该组织成员"


def organization_allows_cross_org_share(organization_id) -> bool:
    """读取 ``Organization.settings.allow_cross_org_share``；默认放开，显式 false 才关闭。"""
    ...
    if not isinstance(settings, dict):
        # 组织存在但没写过 settings：按未配置处理，默认放开
        return True
    value = settings.get(CROSS_ORG_SHARE_SETTING_KEY)
    return value is None or value is True


def is_friend_external_contact(*, owner_user_id: str, peer_user_id: str) -> bool:
    """peer 是否为 owner 已建立关系的外部联系人（relationship=friend）。

    tabchat 走函数内延迟导入，避免模块级循环依赖。
    """
    if not owner_user_id or not peer_user_id:
        return False
    from apps.tabchat.models import ExternalContact

    return ExternalContact.objects.filter(
...
def grantee_can_receive_share(
    *, organization_id, owner_user_id: str, grantee_user_id: str,
) -> bool:
    """判断某人能否作为该组织下会话的共享接收方。

    组织内成员 → 可以；组织外成员 → 仅当组织开放跨组织分享、且他是 owner 的
    外部联系人（friend）时才可以。
    """
    grantee_user_id = str(grantee_user_id or "").strip()
    if not grantee_user_id:
        return False
    if grantee_user_id == str(owner_user_id):
        return False
    ...
    if OrganizationMember.objects.filter(
        ...
        return True
    if not organization_allows_cross_org_share(organization_id):
        return False
    return is_friend_external_contact(
        owner_user_id=str(owner_user_id),
        peer_user_id=grantee_user_id,
    )


def is_share_grantee_eligible(*, share, user_id: str) -> bool:
    """已有授权行的接收方是否仍有资格访问（创建时校验 + 读取时复检同源）。"""
    return grantee_can_receive_share(
        organization_id=share.organization_id,
        owner_user_id=str(share.owner_user_id),
        grantee_user_id=str(user_id or ""),
    )
...
def create_or_update_share(
    *,
    ...
    if not grantee_can_receive_share(
        organization_id=session.organization_id,
        owner_user_id=str(owner_user.id),
        grantee_user_id=grantee_user_id,
    ):
        raise ValueError(GRANTEE_NOT_MEMBER_MESSAGE)
...
def get_active_share(*, session_id, user) -> SessionShare | None:
    ...
    if (
        ...
        return None
    return latest if is_share_grantee_eligible(share=latest, user_id=user.id) else None
...
def get_active_share_by_id_for_user(*, share_id, session_id, user) -> SessionShare | None:
    ...
    if requested.owner_user_id == user_id:
        return requested
    return requested if is_share_grantee_eligible(
        share=requested,
        user_id=user.id,
    ) else None
