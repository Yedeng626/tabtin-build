...
class SessionShareTestCase(TestCase):
    ...
    def _make_friend_external_contact(self):
        from apps.tabchat.models import ExternalContact

        return ExternalContact.objects.create(
            owner_user=self.owner,
            peer_user=self.outsider,
            peer_organization=self.other_org,
            relationship=ExternalContact.Relationship.FRIEND,
        )

    def test_cross_org_share_default_open_allows_friend_contact(self):
        """settings 未配置 allow_cross_org_share 时默认放开：外部好友可作为接收人。"""
        self._make_friend_external_contact()
        share = session_share_service.create_or_update_share(
            session_id=str(self.session.id),
            owner_user=self.owner,
            grantee_user_id=str(self.outsider.id),
        ...
        self.assertEqual(str(share.grantee_user_id), str(self.outsider.id))
        self.assertTrue(
            session_share_service.organization_allows_cross_org_share(
                self.organization.id,
            )
        )

    def test_cross_org_share_explicitly_disabled_rejects_friend_contact(self):
        """显式 false 才关闭：此时外部好友也被拒。"""
        self._make_friend_external_contact()
        self.organization.settings = {
            **(self.organization.settings or {}),
            "allow_cross_org_share": False,
        }
