"""Register JSON-object output for custom chat models.

Only Personal and Organization BYOK models are targeted. Official catalog
metadata remains provider-owned.
"""
...
def merge_custom_chat_json_mode(config: Any) -> dict[str, Any]:
...
def backfill_custom_chat_json_mode(apps, schema_editor):
    ...
    for model in queryset.iterator(chunk_size=500):
        ...
        after = merge_custom_chat_json_mode(before)
        if before == after:
            continue
...
class Migration(migrations.Migration):
    dependencies = [
        ("llm", "0060_glm47_json_mode_capability"),
    ]

    operations = [
        migrations.RunPython(
            backfill_custom_chat_json_mode,
