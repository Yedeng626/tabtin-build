...
def _update_enum_values(apps, schema_editor, forward=True):
    ...
    with schema_editor.connection.cursor() as cursor:
        cursor.execute(
            "UPDATE services_media_provider SET scope = %s WHERE scope = %s", [new, old]
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('media_generation', '0005_alter_mediaprovider_name'),
    ]

    operations = [
        migrations.RenameField(model_name='mediaprovider', old_name='workteam_id', new_name='organization_id'),
        migrations.RenameField(model_name='mediatask', old_name='workteam_id', new_name='organization_id'),
