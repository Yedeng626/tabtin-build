...
def resolve_target_user(identifier: str):
    """按完整手机号或邮箱（大小写不敏感）解析唯一活跃用户。

    单入参按 ``@`` 分流：含 ``@`` 走邮箱精确匹配，其余走手机号等价字面量。
    """
    ...
    return resolve_user_by_phone(value, active_only=True)
...
class ExternalContactService:
    ...
    def discover(
        ...
        target = resolve_target_user(identifier)
