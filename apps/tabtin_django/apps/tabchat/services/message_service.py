...
# 外部会话（is_external，跨组织 DM）的卡片白名单：只放行**任务共享卡**。
# 跨组织共享（Organization.settings.allow_cross_org_share）的唯一投递面就是这张卡；
# 不放开白名单时，共享卡的投影消息会固定命中「外部群仅支持普通消息」，
# 表现为 create-share 502 IM_DELIVERY_REJECTED（error_type=ExternalGroupCapabilityError）。
# 其余富载荷（agent 发送、非文本消息、其它卡片、@Agent）对外部会话仍然一律拒绝。
_CARD_EXTERNAL_ALLOWED_TYPES = frozenset({
    _CARD_SESSION_SHARE_TYPE,
    _CARD_SESSION_SHARE_V2_TYPE,
})


def _is_external_allowed_card(metadata: dict | None) -> bool:
    """metadata.card 是否属于允许进入外部会话的卡片白名单（当前仅任务共享卡）。"""
    card = (metadata or {}).get("card")
    return isinstance(card, dict) and card.get("type") in _CARD_EXTERNAL_ALLOWED_TYPES
    ...
    def send_message(
        ...
        has_external_restricted_payload = bool(
            ...
            and (
                ...
                or (
                    raw_metadata.get("card")
                    and not _is_external_allowed_card(raw_metadata)
                )
            ...
            with transaction.atomic(using=postgres_app_db_alias()):
                ...
                if conv.is_external and not is_system_sender:
                    has_rich_payload = bool(
                        message_type != MessageType.TEXT
                        or (
                            (metadata or {}).get("card")
                            and not _is_external_allowed_card(metadata)
                        )
