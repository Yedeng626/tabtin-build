...
class OrganizationInvitation(models.Model):
    ...
    # 定向邀请（进对方「待处理邀请」、需对方接受）：direct / phone / email。
    # 'email' 有两种形态，以 ``invited_user_id`` 是否为空区分：
    #   - 空   = 令牌式邮件邀请（发给未注册邮箱，走邮件里的链接接受）；
    #   - 非空 = 定向邮箱邀请（对方已注册，走 App 内待处理邀请，与 phone 同链路）。
    TARGETED_INVITE_TYPES = ('direct', 'phone', 'email')
    STATUS_CHOICES = [
        ('pending', '待接受'),
        ('accepted', '已接受'),
        ('rejected', '已拒绝'),
    ...
    invited_user_id = models.CharField(
        ...
        help_text='direct / phone / email 定向邀请时填写目标用户 ID',
    )
