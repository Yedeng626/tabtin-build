...
@router.post("/organizations/{organization_id}/invitations/email-targeted", auth=jwt_auth, response=RESP_CREATE)
def create_email_targeted_invitation(request: HttpRequest, organization_id: UUID, data: InvitationEmailTargetedCreate):
    """按邮箱定向邀请**已注册**用户；对方在 App「待处理邀请」接受，与手机号邀请同链路。"""
    service = InvitationService(user=request.auth)
    try:
        invitation = service.create_email_invitation_for_user(
            organization_id=organization_id,
            email=data.email,
            role=data.role,
            expires_hours=data.expires_hours,
    ...
    _audit(
        ...
        action_type='invitation_create', target_type='invitation',
        target_id=invitation.id, organization_id=organization_id,
        message=f"通过邮箱 {data.email} 邀请用户为 {data.role}",
        activity_action='invited', resource_name=data.email,
    )
