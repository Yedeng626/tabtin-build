...
class InvitationService(BaseService):
    ...
    def create_email_invitation(
        ...
        # 只复用**令牌式**邮件邀请（invited_user_id 为空）：定向邮箱邀请虽然
        # invite_type 同为 'email'，但它是发给某个已注册用户的待处理邀请，
        # 不能被「再发一封邮件邀请」顶掉。邮箱比对不区分大小写。
    ...
    def create_direct_invitation(
        ...
        *,
        ...
        """通过用户 ID（或手机号 / 邮箱解析后的用户）创建定向邀请，目标用户需要手动接受。"""
        if role not in ORGANIZATION_ASSIGNABLE_ROLES:
            raise ServiceError('INVALID_ROLE', f'角色 {role} 不合法')
        ...
        # 定向邮箱邀请把邮箱写进 email 列，供列表/待处理邀请展示受邀人标识。
        normalized_email = (invite_email or '').strip() if invite_type == 'email' else ''
        ...
        if existing:
            ...
            # 同理：再次用邮箱邀请时补齐 email 展示列
            if invite_type == 'email' and normalized_email:
                existing.invite_type = 'email'
                existing.email = normalized_email
                update_fields.extend(['invite_type', 'email'])
    ...
    def create_email_invitation_for_user(
        self,
        organization_id: UUID,
        email: str,
        role: str = 'editor',
        ...
        """通过邮箱邀请**已注册**用户；落库 email 类型 + invited_user_id，接受链路同 direct。

        与 ``create_email_invitation``（令牌式邮件邀请，发给未注册邮箱、走邮件链接）的区别：
        本方法要求邮箱已注册，邀请直接出现在对方 App 的「待处理邀请」里，
        与手机号邀请（``create_phone_invitation``）完全同链路 —— 客户端「组织拉人」用它。
        """
        ...
        if not email:
            raise ServiceError('INVALID_EMAIL', '邮箱不能为空')
        try:
            validate_email(email)
        except ValidationError:
            raise ServiceError('INVALID_EMAIL', '邮箱格式不正确')

        from django.contrib.auth import get_user_model

        # 与登录口径一致：邮箱比对不区分大小写（User.email unique，USERNAME_FIELD='email'）
        User = get_user_model()
        target = (
        ...
        if target is None:
            raise ServiceError('USER_NOT_FOUND_BY_EMAIL', '该邮箱未注册，请先让对方注册', 404)
