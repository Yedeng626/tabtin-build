"""
W1.1 / 统一审批 · UserAgentApprovalMemo 单测专用 settings。
...
- INSTALLED_APPS 只装 W1.1 测试需要的最小集：django 核心 + ``users.auth``
  + ``conversation``（Memo 不再依赖 ChatSession FK，但 settings.py
  会校验 conversation 在 INSTALLED_APPS 中）。

使用方式：
    cd apps/tabtin_django && source venv/bin/activate
    DJANGO_SETTINGS_MODULE=tabtin.settings_approval_memo_test \\
        python manage.py test \\
        apps.users.auth.tests.test_approval_memo_service -v 2

⚠️ CI 注意：默认 ``pytest`` 走主 ``tabtin.settings``，会撞上 conversation/0024
的 MySQL FULLTEXT 失败 → 本文件下的测试**不会**被默认跑到（参见仓库已有
17+ 个 ``settings_*_test.py`` 同样问题）。CI 配置必须**显式新增一行**：
    pytest --ds=tabtin.settings_approval_memo_test \\
        apps/users/auth/tests/test_approval_memo_service.py
否则 W1.1 写的 25+ 个用例等于零覆盖。

未来 W1.2 / W1.3 的 ApprovalMemo 相关测试可以复用本 settings；
如果以后 conversation 0024 加上 ``MysqlOnlyOperation`` wrapper
（或者主测试 conftest 加上 RunSQL skip），这个文件就可以删除。
"""
