...
function buildPersonalPluginRuntimeInstruction(skill: SkillRecord, key: string): string | null {
  ...
  return [
    ...
    '',
    '必须直接调用 `run_terminal_command` 执行下面的 QiZhiKitty CLI 命令：',
    '',
    '```bash',
    command,
    '```',
    ...
    '插件安装目录、local service cwd、项目目录、MCP attach 和浏览器打开都由 QiZhiKitty CLI 转交宿主授权流程，并根据 Personal Plugin registry 自动处理。',
