  ...
  properties: {
    reason: {
      ...
      description:
        ...
        '为什么现在需要重启 QiZhiKitty（会落 telemetry）。典型：用户授予了 macOS 文件访问权限。',
    },
  },
  required: ['reason'],
...
function createRelaunchAppTool(deps: SystemToolsDeps): Tool {
  return {
    ...
    description:
      '重启 QiZhiKitty 宿主进程，让新授权的操作系统权限生效。' +
    ...
    execute: async (input: unknown): Promise<ToolResult> => {
      ...
      if (!deps.relaunchApp) {
        ...
        return {
          content: JSON.stringify({
            ...
            runtime: 'daemon',
            user_message:
              '我现在跑在 QiZhiKitty 后台服务模式下，没法自己重启。请你手动重启一下 QiZhiKitty' +
      ...
      try {
        ...
        return jsonError(
          isUserCancelled
            ? '我刚才请你确认是否要重启 QiZhiKitty（重启后我才能用上你刚授权的权限），你点了取消，所以这次没重启。如果你后来又想让我重启，告诉我一声。'
            : '我尝试在重启前先帮你保存未存改动，但这一步没顺利完成，所以我没有重启 QiZhiKitty（避免动了你的工作）。你可以稍后再让我试一次，或者自己手动重启 QiZhiKitty。',
          {
            ...
            hint: isUserCancelled
              ...
              : 'Do not restart automatically; ask the user to save work or restart QiZhiKitty manually.',
      ...
      return {
        content: JSON.stringify({
          ...
          user_message:
            'QiZhiKitty 正在重启，这次对话会暂时断开几秒钟。重启完成后我会自动重新打开，你可以让我接着试刚才那个被卡住的任务（比如再让我读那个文件夹）。',
          technical_note:
