"""Integrations Lite Pack manifest and skill discovery smoke tests."""
...
def test_manifest_identity(manifest):
    assert manifest["id"] == "tabtin-integrations-lite-pack"
...
def test_discover_app_skills_smoke(manifest):
    ...
    assert all(item.get("version") for item in index)
