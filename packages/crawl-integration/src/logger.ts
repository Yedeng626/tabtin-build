...
import { LogLine } from "./types/logs.js";
...
/**
 * BrowserLogger class that wraps Pino for our specific needs
 *
 * LOGGING PRECEDENCE:
 *
 * Test environments:
 ...
 * This prevents spawning a new worker thread for every Browser instance
 * (which happens when `pino-pretty` transport is used), eliminating the
 * memory/RSS growth observed when many Browser objects are created and
 * disposed within the same process (e.g. a request-per-instance API).
 */
export class BrowserLogger {
  /**
   * Shared Pino logger instance across all BrowserLogger instances.
  ...
  constructor(
    ...
    if (this.usePino) {
      ...
      if (!BrowserLogger.sharedPinoLogger) {
        BrowserLogger.sharedPinoLogger = createLogger(options);
      }
      this.logger = BrowserLogger.sharedPinoLogger;
    }
