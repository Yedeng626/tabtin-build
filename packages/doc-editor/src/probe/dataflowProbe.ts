/**
 * TabDoc 数据流探针（dev-only）
 *
 * 目标：让 AI / 自动化能在真实宿主里观测「用户操作 → 数据变换 → 渲染」这条链路，
 * 而不依赖人工 copy 日志 / 截图。探针只采集事件，host-agnostic：
 *   - 由宿主在 DEV 下显式调用 enableDataflowProbe() 启用（生产包永不启用）；
 *   - 各层（autosave / collab / http / store）通过 recordProbeEvent() 喂事件；
 *   - 宿主注入 sink（Web → POST dev 中间件落文件；Electron → IPC 落文件）。
 *
 * 「区分来源」：每条事件带 origin（user 真人 / agent AI 触发 / probe / system），
 * 共享日志文件里多方在写时可据此摘出各自的数据流，并断言「AI 链路 == 用户链路」。
 */

export type ProbeOrigin = 'user' | 'agent' | 'probe' | 'system'
...
export interface ProbeEvent {
  ...
  /** 节点内的事件名，如 'save.start' / 'save.success' / 'collab.status' / 'http.request' */
  event: string
  host: ProbeHost
  /** 跨层/跨服务关联 id（接 ipc-shim / HTTP header 的 trace_id，串联服务端日志） */
  traceId?: string
  /** 一次浏览器会话/驱动批次 id，用于把一次验证跑批从别人的事件里摘出来 */
  sessionId?: string
  /** 关联的文档 id（若有） */
  docId?: string
...
/** 注入 sink（宿主决定落到哪：dev 中间件文件 / IPC 文件 / console）。 */
...
/** 注销一个意图（编辑器卸载时调用，避免残留指向已卸载组件的 handler）。 */
 ...
 * 以便和用户手点(origin='user')区分。意图内若触发异步保存，handler 应自行 flush。
 */
export const fireProbeIntent = async (
  ...
  if (!intent) {
    throw new Error(
      `[tabdocProbe] 未知 intent: ${name}。可用: ${Array.from(state.intents.keys()).join(', ') || '(无)'}`,
    )
  }
...
export const dumpProbeEvents = (filter: ProbeDumpFilter = {}): ProbeEvent[] =>
  state.buffer.filter((e) => {
    ...
    if (filter.docId && e.docId !== filter.docId) return false
    return true
  })
...
const HELP_TEXT = `window.__tabdocProbe — TabDoc 数据流探针 (dev-only)
  fireIntent(name, args?)  触发 UI 同款意图(标 origin=agent)，可用 intent 见 listIntents()
  listIntents()            列出可触发的意图
  dump(filter?)            读取数据流事件 {since,origin,component,traceId,docId}
  clear()                  清空 ring buffer
  flush()                  立即把事件刷到 sink(落日志文件)
  sessionId()              当前会话 id(用于在共享日志里摘出本次跑批)
事件字段: {ts,origin,component,event,host,traceId,sessionId,docId,payload}
  origin: user 真人 / agent AI触发 / probe / system`
...
const attachWindowApi = (): void => {
  const g = globalThis as unknown as { __tabdocProbe?: WindowProbeApi }
  if (g.__tabdocProbe) return
  g.__tabdocProbe = {
