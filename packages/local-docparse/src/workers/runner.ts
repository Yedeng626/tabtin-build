...
export type QueueStrategy = 'fifo' | 'coalesce' | 'latest-wins'
...
export interface WorkerTaskRunnerOptions {
  workerScriptPath: string
  concurrency: number
  name?: string
  debug?: boolean
  /** 空闲 Worker 自动终止的超时时间（毫秒），0 表示不缩容。默认 30_000 */
...
export class WorkerTaskRunner {
  ...
  runTask<TResult>(
    ...
    if (this.disposed) {
      return Promise.reject(new WorkerTaskError(`[${this.name}] Runner has been disposed`))
    }
    ...
    const generation =
      useGeneration && dedupeKey ? (this.generationByKey.get(dedupeKey) ?? 0) + 1 : 0
    ...
    const taskPromise = new Promise<TResult>((resolve, reject) => {
      ...
      if (task.abortSignal) {
        const abortTask = (): void => this.abortTask(task.taskId)
    ...
    return taskPromise.finally(() => {
      ...
      if (useGeneration && dedupeKey && this.generationByKey.get(dedupeKey) === generation) {
  ...
  async dispose(): Promise<void> {
    ...
    for (const taskId of [...this.queue]) {
      this.rejectTask(taskId, new WorkerTaskAbortedError('Worker runner disposed'))
    }
    this.queue.length = 0
    ...
    for (const slot of this.workerSlots.values()) {
      ...
      if (slot.activeTaskId) {
        this.rejectTask(slot.activeTaskId, new WorkerTaskAbortedError('Worker runner disposed'))
      }
  ...
  private spawnWorker(): void {
    ...
    const slot: WorkerSlot = { id: slotId, worker, activeTaskId: null, terminating: false }

    worker.on('message', (message: unknown) => this.handleWorkerMessage(slot.id, message))
    ...
    worker.on('exit', (code) => {
      ...
      if (code !== 0) this.log(`worker ${slot.id} exited with code ${code}`)
  ...
  private drainQueue(): void {
    ...
    for (const slot of this.workerSlots.values()) {
      ...
      task.slotId = slot.id
      task.timeoutHandle = setTimeout(() => this.handleTaskTimeout(task.taskId), task.timeoutMs)
  ...
  private handleWorkerMessage(slotId: number, message: unknown): void {
    ...
    if (!this.isWorkerResultMessage(message)) return
    const response = message

    if (slot.activeTaskId !== response.taskId) {
      this.log(`worker ${slot.id} sent unexpected result ${response.taskId}`)
      return
    }
    ...
    if (response.ok) {
      if (
        ...
        this.rejectTask(task.taskId, new WorkerTaskAbortedError('Task superseded'))
      } else {
        this.resolveTask(task.taskId, response.result)
      }
    } else {
      this.rejectTask(task.taskId, new WorkerTaskError(response.error.message, response.error))
    }
  ...
  private scheduleIdleCleanup(): void {
    ...
    for (const slot of this.workerSlots.values()) {
      if (slot.terminating) continue
      if (slot.idleTimer) clearTimeout(slot.idleTimer)

      if (!slot.activeTaskId && this.queue.length === 0) {
        slot.idleTimer = setTimeout(() => {
          if (slot.activeTaskId || slot.terminating) return
          this.log(`terminating idle worker ${slot.id}`)
          slot.terminating = true
  ...
  private isWorkerResultMessage(message: unknown): message is WorkerTaskResponseMessage {
    if (!message || typeof message !== 'object') return false
    const m = message as Record<string, unknown>
    if (m.kind !== 'result' || typeof m.taskId !== 'string' || typeof m.ok !== 'boolean') {
      return false
    }
    if (m.ok) return true
    if (!m.error || typeof m.error !== 'object') return false
  ...
  private handleTaskTimeout(taskId: string): void {
    ...
    this.rejectTask(
      taskId,
      new WorkerTaskError(`[${this.name}] Task "${task.taskType}" timed out after ${task.timeoutMs}ms`),
    )
  ...
  private handleWorkerFailure(slotId: number, error: WorkerTaskError): void {
    ...
    if (activeTaskId) this.rejectTask(activeTaskId, error)
  ...
  private dropQueuedTasksByKey(dedupeKey: string, keepTaskId: string): void {
    for (let i = this.queue.length - 1; i >= 0; i--) {
      ...
      if (!queuedTask || queuedTask.taskId === keepTaskId || queuedTask.dedupeKey !== dedupeKey) {
        continue
      }
      this.queue.splice(i, 1)
      this.rejectTask(queuedTask.taskId, new WorkerTaskAbortedError('Task superseded'))
    }
  }
  ...
  private rejectTask(taskId: string, reason: unknown): void {
    ...
    if (slot?.activeTaskId === taskId) slot.activeTaskId = null
