/**
 * LSP Client —— spawn LSP server 子进程 + vscode-jsonrpc 通信封装。
 *
 * 设计要点（与既定 LSP 语义一致）：
 *   - 必须等 'spawn' 事件再用 stdin/stdout（避免 ENOENT 异步错误）
 *   - 4 类错误处理：connection.onError / connection.onClose /
 *     process.on('exit') / process.stdin.on('error')，全部 catch + log，
 *     绝不 unhandled rejection
 *   - pendingHandlers 队列：connection 没 ready 时 onNotification 可以先入队
 *   - shutdown 协议：'shutdown' request + 'exit' notification + dispose + kill
 */
  ...
  return {
      ...
      try {
        ...
        process.on('exit', (code, _signal) => {
          if (code !== 0 && code !== null && !isStopping) {
            ...
            logError(crashError.message, crashError);
            onCrash?.(crashError);
          }
        });
        ...
        logError(
          `LSP server ${serverName} failed to start: ${err.message}`,
          err,
        );
        throw error;
      }
    ...
    async stop(): Promise<void> {
      ...
      try {
        ...
        const err = error as Error;
        logError(`LSP server ${serverName} stop failed: ${err.message}`, err);
        shutdownError = err;
