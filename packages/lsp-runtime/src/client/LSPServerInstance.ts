/**
 * LSPServerInstance —— 单个 LSP server 的生命周期管理。
 *
 * 设计要点（与既定 LSP 语义一致）：
 *   - 状态机：stopped → starting → running → stopping → stopped，错误 → error
 *   - 完整 InitializeParams（声明 publishDiagnostics / synchronization.didSave
 *     等 client capability）
 *   - transient error 重试：LSP_ERROR_CONTENT_MODIFIED = -32801，3 次指数退避
 *   - maxRestarts 默认 3
 *   - crash 回调 onCrash 让上层 ensureServerStarted 可重启
 */
...
import type { LspServerState, ScopedLspServerConfig } from '../manager/types.js';
...
export function createLSPServerInstance(
  ...
  async function start(): Promise<void> {
    ...
    try {
      ...
      logError((error as Error).message, error as Error);
      throw error;
    }
  }
  ...
  async function restart(): Promise<void> {
    try {
      ...
      logError(stopError.message, stopError);
      throw stopError;
    }
    ...
    try {
      ...
      logError(startError.message, startError);
      throw startError;
    }
  }
  ...
  async function sendRequest<T>(method: string, params: unknown): Promise<T> {
    ...
    logError(requestError.message, requestError);
    throw requestError;
  }
  ...
  async function sendNotification(
    ...
    try {
      ...
      logError(notificationError.message, notificationError);
