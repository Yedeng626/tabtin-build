/**
 * LSPServerManager —— 多 LSP server 路由 + 文件同步。
 *
 * 设计要点（与既定 LSP 语义一致）：
 *   - 按 `extensionToLanguage` 路由：扩展名 → server name → server instance
 *   - `ensureServerStarted` 懒启动：第一次需要时才 spawn
 *   - `openedFiles: Map<URI, serverName>` 跟踪：必须 didOpen 才能 didChange
 *   - 4 个 LSP 通知封装：openFile / changeFile / saveFile / closeFile
 *   - `changeFile` 见 server.state !== 'running' 自动 fallback 到 openFile
 *   - `closeAllFiles`（compact 时调用）
 *   - workspace/configuration request handler（某些 server 即使 client 声明
 *     不支持也会发，回 null 数组满足协议）
 */
...
} from '../client/LSPServerInstance.js';
...
export interface LspServerConfigLoader {
  load(): Promise<{ servers: Record<string, ScopedLspServerConfig> }>;
}
...
export function createLSPServerManager(
  ...
  async function initialize(): Promise<void> {
    ...
    try {
      ...
      logForDebugging(
        `[LSP SERVER MANAGER] configLoader returned ${Object.keys(serverConfigs).length} server(s)`,
      );
    } catch (error) {
      const err = error as Error;
      logError(`Failed to load LSP server configuration: ${err.message}`, err);
      throw error;
    }
