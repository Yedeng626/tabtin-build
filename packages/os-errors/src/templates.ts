...
function tplPermissionDeniedMacOS(input: TemplateInput): TemplateOutput {
  ...
  const headBase =
    ...
    `请打开「${settingsPath}」，找到 QiZhiKitty，`;

  if (isFullDisk) {
    // Full Disk Access：授权后必须重启
    const userGuidance =
      headBase +
      `把它加入并勾选启用。授权后 QiZhiKitty 会弹出确认框，让你打开系统设置或重启应用；重启前会先请你保存未存工作。`;
    ...
    const recoveryActions: RecoveryAction[] = [
      ...
      { type: 'restart_app', label: '重启 QiZhiKitty（完全磁盘访问授权后必须）' },
  ...
  const userGuidance =
    headBase +
    `打开「${cat}」开关。授权完后告诉我一声，我直接帮你接着读，通常不需要重启 QiZhiKitty。`;

  const agentDirectives = [
    '不要重试这个路径，重试只会再次拿到同样的权限错误',
    '不要建议用户用 sudo / chmod / xattr 等方式绕过 macOS 权限保护',
    '用户表示授权完成后，立即重试原工具；不要假设存在模型侧解封工具',
    '只有当重试再次拿到同一权限错误时（极少见），才说"看起来这条权限可能需要重启 QiZhiKitty 才生效，要我帮你重启吗？"，用户同意时交给宿主授权流程处理',
...
function tplAVBlocked(input: TemplateInput): TemplateOutput {
  const { path: p } = input;
  const userGuidance =
    `我访问 ${p} 时操作被拦截或长时间没响应，疑似你电脑上的安全软件（常见为 360 安全卫士 / 火绒 / 腾讯电脑管家 / Windows Defender 受控文件夹访问）阻止了 QiZhiKitty。` +
    `请打开你的安全软件，把 QiZhiKitty 加进信任 / 白名单后告诉我，我重新尝试。` +
    `如果不确定用的是哪个安全软件，看任务栏右下角的盾牌或动物图标。`;

  const agentDirectives = [
    `不要重试这个路径直到用户确认已加白名单`,
    `不要建议重启 QiZhiKitty —— 重启不解决杀软拦截`,
    `如果用户使用 Windows Defender 受控文件夹访问，可推荐快捷链接：windowsdefender://Threat/`,
  ];

  const recoveryActions: RecoveryAction[] = [
    { type: 'whitelist_in_av', label: '在安全软件中添加 QiZhiKitty 白名单' },
    {
