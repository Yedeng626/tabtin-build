...
const TMALL_LOGIN_HINT =
  ...
  '请在 QiZhiKitty 浏览器淘宝/天猫标签扫码登录后重试；与淘宝同属阿里系会话。'
...
const TAOBAO_LOGIN_HINT =
  '淘宝 PC 搜索匿名不出货（不发 wsearch）。请在 QiZhiKitty 浏览器当前淘宝标签扫码登录后重试；批量低频，易滑块/风控。'
...
const JD_LOGIN_HINT =
  '请在 QiZhiKitty 浏览器京东标签登录后重试；撞 risk_handler / 验证码时先在页内完成再 search。'
