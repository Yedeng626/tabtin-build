/**
 * shell 命令写副作用判定 —— 供 judge / HITL risk 共用。
 *
 * AH-005：agent-runtime 不得依赖本包；runtime 侧有同语义拷贝
 * packages/agent-runtime/src/capability/core/shell-command-side-effect.ts。
 * 改一侧时务必同步另一侧。
 *
 * 原则：
 ...
 *   - stdout 重定向 `>` / `>>`（不含 `2>` / `&>`）→ 当写
 *
 * 与 tool-orchestration 的 READONLY_COMMAND_RE 对齐并扩展 jq 等只读工具头。
 */
