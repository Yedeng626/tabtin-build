...
description: >
  ...
  QiZhiKitty App 登录并加入此 Space，不要反复重试。
metadata:
  version: 0.2.0
  tabtin:
    category: device
...
- **用户没有连过手机，只在桌面客户端**——本 Skill 用不上，请直接告诉用户“此功能需要先在手机端 QiZhiKitty App 登录并加入当前组织”
...
通过 `run_terminal_command` 执行 `tabtin device …` 命令。CLI 内部走 cli-server（Electron 主进程或 Daemon 进程），最终调 Django `/api/tabtinspace/devices/query` 派发到 Space 的能力设备。**两端是同一条派发链路**——CLI 调用结果跟 FC 工具调用结果等价，只是入口不同。
...
```bash
tabtin device info    [--space-id <id>] [--timeout 30] [--format json]
tabtin device battery [--space-id <id>] [--timeout 30] [--format json]
tabtin device network [--space-id <id>] [--timeout 30] [--format json]
```
...
`tabtin device …` 失败时 `run_terminal_command` 看到的 stderr 是 **Go CLI 又包了一层** 的双层 JSON：
...
| `TASK_FAILED` | 409 | **该 Space 没有绑定且在线的能力设备**（cli-server 把后端 `DEVICE_RUNTIME_UNAVAILABLE` / `DEVICE_RUNTIME_OFFLINE` 都映射成这个码） | **不要再调任何 device 工具**。直接告诉用户：当前工作空间还没有可用的手机；需要在 iOS / Android 端打开 QiZhiKitty App，登录并加入当前组织，确保 App 在前台或保持后台运行权限。等用户确认连上后再重试一次 |
...
| `AUTH_MISSING` | 401 | 未认证 / token 过期 | 告诉用户重新登录 QiZhiKitty |
