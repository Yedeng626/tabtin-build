...
export type RowDropMode = 'before' | 'after' | 'inside';
...
export const getRowDropTarget = (
  ...
  const hoverRowOffsetY = coordInstance.getRowOffset(hoverRowIndex);
  const hoverRowHeight = coordInstance.getRowHeight(hoverRowIndex);
  const beforeHalf = inRange(
    offsetY,
    hoverRowOffsetY,
    hoverRowOffsetY + hoverRowHeight / 2
  );
  const dropIndex = beforeHalf ? hoverRowIndex : hoverRowIndex + 1;
  const relativeY = hoverRowHeight > 0
    ? (offsetY - hoverRowOffsetY) / hoverRowHeight
    : 0;
  ...
  if (relativeY >= 0.25 && relativeY <= 0.75) {
    return {
      dropIndex,
      mode: 'inside',
      targetIndex: hoverRowIndex,
    };
  }

  return beforeHalf
    ? { dropIndex, mode: 'before', targetIndex: hoverRowIndex }
    : { dropIndex, mode: 'after', targetIndex: hoverRowIndex };
};
...
export const useDrag = (
  ...
  const onDragStart = (
    ...
    } else if (type === RegionType.ColumnHeader && draggable !== DraggableType.Row) {
