...
} from './selectionContextMenuUtils';
...
export const useSelection = (props: IUseSelectionProps) => {
  ...
  const onSelectionChange = (mouseState: IMouseState) => {
    ...
    if (isCellSelection && ranges[0] && !selection.equals([ranges[0], newRange])) {
  ...
  const onSelectionContextMenu = (
    ...
    const isRowHeaderHovered = isRowHeaderContextMenuRegion(type, columnIndex);
    ...
    if (isCellHovered) {
      ...
      if (isRowHeaderHovered) {
        if (selectable !== SelectableType.All && selectable !== SelectableType.Row) {
          return;
        }
        const range = buildSingleRowSelectionRange(rowIndex);
        const newSelection = selection.set(SelectionRegionType.Rows, [range]);
        setActiveCell(null);
        setSelection(newSelection);
