...
export class CoordinateManager implements ICoordinate {
  ...
  private _rowHeightMapVersion = 0;
  ...
  public get totalWidth() {
    if (this.columnCount <= 0) return this.columnInitSize;
    const { offset, size } = this.getCellMetaData(this.columnCount - 1, ItemType.Column);
    return offset + size;
  }

  public get totalHeight() {
    if (this.rowCount <= 0) return this.rowInitSize;
  ...
  protected getCellMetaData(index: number, itemType: ItemType): ICellMetaData {
    ...
    const metadata = cellMetadataMap[index];
    if (!metadata) {
      const itemCount = isColumnType ? this.columnCount : this.rowCount;
      console.warn(
        `[CoordinateManager] getCellMetaData: index ${index} out of bounds (count: ${itemCount})`
      );
  ...
  public get rowHeightMapVersion() {
    return this._rowHeightMapVersion;
  }
  ...
  public refreshRowDimensions({
    ...
    rowInitSize = 0,
    rowHeightMap = {},
  }: Pick<ICoordinate, 'rowCount' | 'pureRowCount' | 'rowInitSize' | 'rowHeightMap'>) {
    this.rowCount = rowCount;
    this.pureRowCount = pureRowCount;
    this.rowInitSize = rowInitSize;
    this.rowHeightMap = rowHeightMap;
