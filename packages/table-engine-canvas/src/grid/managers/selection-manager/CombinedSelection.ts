...
export class CombinedSelection {
  ...
  public set(type: SelectionRegionType, ranges: IRange[]) {
    ...
    if (type === SelectionRegionType.Cells && ranges.length < 2) {
      throw Error('Ranges of type cells should have a length equal to 2!');
    }

    if ([SelectionRegionType.Columns, SelectionRegionType.Rows].includes(type) && !ranges.length) {
      throw Error('Ranges of type columns or rows should have a length of at least 1!');
    }

    if (ranges.some((r) => r == null)) {
      throw Error('Ranges should not contain null or undefined elements!');
    }
