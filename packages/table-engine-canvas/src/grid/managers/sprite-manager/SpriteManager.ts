  ...
  public drawSprite(ctx: CanvasRenderingContext2D, props: ISpriteDrawerProps) {
    ...
    if (spriteCanvas === undefined) {
      ...
      promise
        ...
        .catch(() => {
          // Keep the empty canvas in cache to prevent infinite retry loops.
          // A failed sprite renders as blank rather than causing continuous
          // bumpForceRender → full redraw → retry → fail cycles.
        })
        .finally(() => {
          this.inFlight--;
          if (this.inFlight === 0) {
