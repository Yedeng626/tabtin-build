...
const MAX_MULTILINE_CHARS = 2000;
...
export const drawMultiLineText = (ctx: CanvasRenderingContext2D, props: IMultiLineTextProps) => {
  ...
  const cacheKey = `${fontKey}|${text}|${fontSize}|${lineHeight}|${maxWidth}|${maxLines}`;
  const cachedLines = multiLineTextInfoCache.get(cacheKey);

  if (cachedLines) {
    lines = cachedLines;
  } else {
    const safeLength = Math.min(text.length, MAX_MULTILINE_CHARS);
    const textTruncated = safeLength < text.length;
      ...
      if (currentLineWidth + charWidth > maxWidth) {
        if (lines.length < maxLines - 1) {
          ...
          if (currentLineWidth + ellipsisWidth > maxWidth) {
            ...
            while (tempLineWidth + ellipsisWidth > maxWidth && tempLine.length > 0) {
              const removedChar = tempLine[tempLine.length - 1];
              tempLine = tempLine.substring(0, tempLine.length - 1);
              tempLineWidth -= ctx.measureText(removedChar).width;
            }
...
export const drawSingleLineText = (ctx: CanvasRenderingContext2D, props: ISingleLineTextProps) => {
  ...
  const cacheKey = `${ctx.font}|${text}|${fontSize}|${maxWidth}`;
  ...
  if (cachedTextInfo) {
    ...
    if (isDisplayEllipsis) {
      while (width + ellipsisWidth > maxWidth && displayText.length > 0) {
        const removedChar = displayText[displayText.length - 1];
        displayText = displayText.slice(0, -1);
        width -= ctx.measureText(removedChar).width;
      }
...
export const drawRect = (ctx: CanvasRenderingContext2D, props: IRectProps) => {
  ...
  if (_radius == null) {
    ...
    const maxRadius = Math.min(Math.abs(height) / 2, Math.abs(width) / 2);
    const clampR = (r: number) => Math.max(0, Math.min(r, maxRadius));
    const radius =
      typeof _radius === 'number'
        ? { tl: clampR(_radius), tr: clampR(_radius), br: clampR(_radius), bl: clampR(_radius) }
        : {
            tl: clampR(_radius.tl),
            tr: clampR(_radius.tr),
            bl: clampR(_radius.bl),
            br: clampR(_radius.br),
          };
