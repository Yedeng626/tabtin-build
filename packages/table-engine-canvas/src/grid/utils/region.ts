...
const checkIsCommentCount = (props: ICheckRegionProps): IRegionData | null => {
  const {
    position,
    columns,
    commentCountMap,
  ...
  const { rowIndex, columnIndex, x, y } = position;
  if (rowIndex < 0 || columnIndex < 0 || !columns[columnIndex]?.isPrimary) return null;
  ...
  if (!resolveCellCommentCount(cell, columns[columnIndex], commentCountMap)) return null;

  const bounds = getCommentCountBounds({
    x: coordInstance.getColumnRelativeOffset(columnIndex, scrollState.scrollLeft),
    y: coordInstance.getRowOffset(rowIndex) - scrollState.scrollTop,
  ...
  return { ...bounds, type: RegionType.CommentCount, rowIndex, columnIndex };
};
...
/**
 * Detect tree expand arrow and hover action buttons in the first column content area.
 * Tree arrow: in the indent space, matching draw position in drawCellContent.
 * Action buttons (expand detail, add sub-record): right side of first column on hover.
 */
const checkIsCellActionArea = (props: ICheckRegionProps): IRegionData | null => {
  const {
    coordInstance,
    position,
    scrollState,
  ...
  const cellX = coordInstance.getColumnRelativeOffset(0, scrollLeft);
  const cellY = coordInstance.getRowOffset(rowIndex) - scrollTop;
  const cellWidth = coordInstance.getColumnWidth(0);
  const rowHeight = coordInstance.getRowHeight(rowIndex);

  const treeData = props.getRowTreeData?.(linearRow.realIndex);
  const isInTreeMode = typeof treeData?.treeDepth === 'number';

  // Tree expand/collapse arrow hit area (in the indent space)
  if (isInTreeMode && treeData?.treeHasChildren) {
    const arrowCenterX = cellX + treeData!.treeDepth! * TREE_INDENT_PER_LEVEL + TREE_INDENT_PER_LEVEL / 2;
    const arrowCenterY = cellY + rowHeight / 2;
    const hitSize = 14;
    const halfHit = hitSize / 2;

    if (
      inRange(x, arrowCenterX - halfHit, arrowCenterX + halfHit) &&
      inRange(y, arrowCenterY - halfHit, arrowCenterY + halfHit)
    ) {
      return {
        type: RegionType.RowTreeExpandHandler,
        x: arrowCenterX - halfHit,
        y: arrowCenterY - halfHit,
        width: hitSize,
        height: hitSize,
        rowIndex,
      };
  ...
  // Hover action buttons (right side of first column)
  const btnSize = 16;
  const btnGap = 4;
  const btnY = cellY + (rowHeight - btnSize) / 2;

  // "查看" (expand record detail) button — before the comment tag when present
  const firstCell = getCellContent([0, linearRow.realIndex]);
  ...
  let btnX = getRowDetailButtonBounds(
    { x: cellX, y: cellY, width: cellWidth, height: rowHeight },
    hasCommentCount,
  ).x;
  if (inRange(x, btnX, btnX + btnSize) && inRange(y, btnY, btnY + btnSize)) {
    return {
  ...
  // "添加子记录" button — left of expand (tree mode only)
  if (isInTreeMode) {
    btnX -= btnSize + btnGap;
    if (inRange(x, btnX, btnX + btnSize) && inRange(y, btnY, btnY + btnSize)) {
      return {
        type: RegionType.RowTreeAddSubRecord,
        x: btnX,
        y: btnY,
        width: btnSize,
        height: btnSize,
