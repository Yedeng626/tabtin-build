...
export const API_ENDPOINTS = {
  ...
  ORGANIZATION_INVITATION: {
    ...
    // 按邮箱定向邀请**已注册**用户（服务端 invited_user_id 非空的 email 类型邀请）。
    // 与 CREATE_EMAIL（令牌式邮件邀请，发给未注册邮箱、走邮件里的链接）不是一回事。
    CREATE_EMAIL_TARGETED: (organizationId: string) =>
      `/context/organizations/${organizationId}/invitations/email-targeted`,
