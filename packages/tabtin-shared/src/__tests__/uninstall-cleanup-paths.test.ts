...
describe('uninstall-cleanup-paths', () => {
  ...
  it('updater cache paths sit under local cache root', () => {
    ...
    // 新品牌优先；历史 TabTin 目录仍留在清单里，老安装的残留才清得掉
    expect(caches[0]).toBe(path.join(localCache, 'com.qizhikitty.community-updater'))
    expect(caches).toContain(path.join(localCache, 'com.tabtin.app-updater'))
  })

  it('mac app bundles are under /Applications', () => {
    const bundles = resolveMacAppBundlePaths()
    expect(bundles).toContain(path.join('/Applications', 'QiZhiKitty.app'))
    // 历史 TabTin.app 仍可被卸载助手移入废纸篓
    expect(bundles).toContain(path.join('/Applications', 'TabTin.app'))
  })
