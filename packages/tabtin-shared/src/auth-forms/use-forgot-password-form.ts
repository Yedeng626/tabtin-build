...
export function useForgotPasswordForm(
  ...
  const setField = (field: ForgotField, value: string) => {
    ...
    const next = field === 'username' ? sanitizeAuthIdentifierInputByMode(value, identifierMode) : value
  ...
  const validateRequest = (): boolean => {
    ...
    if (!username.trim()) {
      errors.username = t(identifierCopy.forgotRequired)
    } else if (!isValidAuthIdentifierByMode(username, identifierMode)) {
      errors.username = t(
        identifierMode === 'emailOrPhone' && !username.includes('@')
          ? 'forgotForm.errors.usernameInvalid'
          : identifierCopy.forgotInvalid,
      )
    }
  ...
  const submitRequest = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setTimeout(() => {
        setSuccessMessage(resp?.message || t(identifierCopy.forgotCodeSent))
      }, 100)
    } catch (err) {
