...
export interface UseLoginFormDeps {
  ...
  /** 手机号登录开关，默认 true；传 false = 只允许邮箱作为标识符 */
...
export function useLoginForm(deps: UseLoginFormDeps): UseLoginFormResult {
  ...
  const phoneLoginEnabled = deps.phoneLoginEnabled ?? true
  const identifierMode = resolveAuthIdentifierMode(emailLoginEnabled, phoneLoginEnabled)
  const identifierCopy = resolveAuthIdentifierCopy(identifierMode)
  ...
  const setField = (field: 'username' | 'password' | 'verificationCode', value: string) => {
    let next = value
    if (field === 'username') {
      next = sanitizeAuthIdentifierInputByMode(value, identifierMode)
  ...
  const switchMethod = (next: LoginUiMethod) => {
    ...
    resetFeedback()
    setMethod(next)
    setUsername((prev) => sanitizeAuthIdentifierInputByMode(prev, identifierMode))
  }
  ...
  const validate = (): boolean => {
    ...
    if (!trimmedUsername) {
      errors.username = authFeedbackKey(
        isVerification ? identifierCopy.loginRequiredBeforeCode : identifierCopy.loginRequired,
      )
    } else if (!isValidAuthIdentifierByMode(trimmedUsername, identifierMode)) {
      errors.username = authFeedbackKey(
        identifierMode === 'emailOrPhone' && !trimmedUsername.includes('@')
          ? 'loginForm.errors.phoneInvalid'
          : identifierCopy.loginInvalid,
      )
    }
  ...
  const sendCode = async (): Promise<void> => {
    ...
    if (!trimmedUsername) {
      setFieldFeedbacks({
        username: authFeedbackKey(identifierCopy.loginRequiredBeforeCode),
      })
      return
    }
    if (!isValidAuthIdentifierByMode(trimmedUsername, identifierMode)) {
      setFieldFeedbacks({
    ...
    try {
      ...
      startCountdown()
      setSuccessMessage(
        resp?.message || t(identifierCopy.loginCodeSent),
      )
      setFieldFeedbacks({})
    } catch (err) {
