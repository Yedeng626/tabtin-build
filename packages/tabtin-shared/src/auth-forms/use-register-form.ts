...
export function useRegisterForm(deps: UseRegisterFormDeps): UseRegisterFormResult {
  ...
  const setField = (field: RegisterField, value: string) => {
    ...
    const next = field === 'phone' ? sanitizeAuthIdentifierInputByMode(value, identifierMode) : value
  ...
  const validate = (): boolean => {
    ...
    if (!trimmedPhone) {
      errors.phone = authFeedbackKey(identifierCopy.registerRequired)
    } else if (!isValidAuthIdentifierByMode(trimmedPhone, identifierMode)) {
      errors.phone = authFeedbackKey(
        identifierMode === 'emailOrPhone' && !trimmedPhone.includes('@')
          ? 'registerForm.errors.phoneInvalid'
          : identifierCopy.registerInvalid,
      )
    }
  ...
  const sendCode = async (): Promise<void> => {
    ...
    if (!trimmedPhone) {
      setFieldFeedbacks({
        phone: authFeedbackKey(identifierCopy.registerRequired),
      })
      return
    }
    if (!isValidAuthIdentifierByMode(trimmedPhone, identifierMode)) {
      setFieldFeedbacks({
    ...
    try {
      ...
      startCountdown()
      setSuccessMessage(
        resp?.message || t(identifierCopy.registerCodeSent),
      )
      setFieldFeedbacks({})
    } catch (err) {
