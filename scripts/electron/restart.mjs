...
export function resolveFrontendCommand(
  ...
  if (platform === 'win32') {
    return {
      ...
      args: ['/d', '/s', '/c', 'call', 'scripts\\electron\\restart.bat'],
    };
  }
  return {
    command: 'bash',
    args: [path.join(root, 'scripts', 'electron', 'restart.sh')],
  };
}
...
export function runFrontend({
  ...
  const plan = resolveFrontendCommand(platform, root);
